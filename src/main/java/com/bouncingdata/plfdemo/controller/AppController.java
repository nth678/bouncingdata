...
public class AppController {
      ...
      if (visuals != null) {
        ...
        for (Visualization v : visuals) {
          if ("html".equals(v.getType())) {
            visualsMap.put(v.getName(), new VisualizationDetail(v.getGuid(), "visualize/app/" + guid + "/" 
                + v.getGuid() + "/html", VisualizationType.getVisualType(v.getType())));
          } else if ("png".equals(v.getType())) {
            try {
              String source = appStoreService.getVisualization(guid, v.getGuid(), v.getType());
              visualsMap.put(v.getName(), new VisualizationDetail(v.getGuid(), source, 
  ...
  public @ResponseBody ExecutionResult executeApp(@PathVariable String appGuid, @RequestParam(value = "code", required = true) String code,
    ...
    try {
      ...
      try {
        ...
        String data = logmapper.writeValueAsString(new String[] { "3", appGuid, "", type });
      ...
      if (script == null)
        return new ExecutionResult(null, null, 0, 0, -1, "Application not found.");

      if (script.getUser().getId() != user.getId()) {
        return new ExecutionResult(null, null, 0, 0, -1, "No permission to run this app.");
      }
      ...
      /*
       * if (!script.isExecuted() && script.isPublished() && script instanceof
       * Analysis) { script.setExecuted(true);
      ...
      if ("python".equals(script.getLanguage())) {
        result = appExecutor.executePython(script, code, user);
      } else if ("r".equals(script.getLanguage())) {
        result = appExecutor.executeR(script, code, user);
      } else {
        return new ExecutionResult(null, null, 0, 0, 1, "Not supported language.");
      }
      ...
      try {
        ...
        logger.debug("Failed to save last output of script execution", e);
      }
      return result;

    } catch (Exception e) {
      logger.error("Execution error", e);
      return new ExecutionResult(null, null, 0, 0, 999, "Unknown error");
    }
  ...
  public @ResponseBody String updateDashboard(@PathVariable String guid, @RequestParam(value = "status", required = true) String status,
    ...
    try {
      ...
      /*
       * if (app.isPublished() && "execute".equals(cause)) { // create activity
  ...
  public @ResponseBody void publishAnalysis(@RequestParam(value = "value", required = true) boolean value, @PathVariable String guid,
